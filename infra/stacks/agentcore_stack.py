"""AgentCore stack — IAM execution role, S3 bucket.

Defines the IAM role that AgentCore containers assume and the S3 bucket for
per-user workspace persistence. VPC/Security Group removed (AgentCore Runtime
is AWS-managed; user VPC networking unused).
"""
...
class HermesAgentCoreStack(Stack):
    """IAM role, S3 user-files bucket for AgentCore."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # AWS Marketplace — required by Bedrock to verify Anthropic model subscription.
        # Includes all Marketplace actions for safety; PoC scope.
        self.execution_role.add_to_policy(
            iam.PolicyStatement(
        ...
        # Extra Bedrock permissions for cross-region inference profiles.
        self.execution_role.add_to_policy(
            iam.PolicyStatement(
                sid="BedrockExtra",
                actions=[
